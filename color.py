...
class ColorSystem(IntEnum):
    """either of the 3 colors supported by the terminals"""
    STANDARD=1
    EIGHT_BIT=2
    FULL=3
...
class ColorParseError(Exception):
    """when the color could not be parsed"""


RE_COLOR=re.compile(
    r"""^
\#([0-9a-f]{6})$|  #123bce....
([0-9]{1,3})$|     #123 or 1 or 09....
rgb\(([\d\s,]+)\)$  #rgb(0-255,0-255,0-255)
""",
    re.VERBOSE,
)
...
class Color(NamedTuple):
    ...
    def parse(cls,color:str)->Optional[Color]:
        ...
        #if given default param , just return color obj with default color type
        if color=="default":
            return cls(color,type=ColorType.DEFAULT)

        #if its one of the standard colors return a color obj with standart type
        ...
        #if supplied color doesnt meets on of the 3 patterns return None
        ...
        if color_8:
            ...
            if not all(components <=255 for components in triplet):
                raise ColorParseError(f"color components must be <=250 in {color!r}" )
    ...
    def _match_color(
        ...
        """Find a color from a pallette that mostly  closely matches a given color"""
        ...
        def get_color_distance(color:Tuple[int,int,int])->float:
            ...
            return distance
        min_index,min_color=min(
            enumerate(pallette),
            key=lambda pallete_color: get_color_distance(pallete_color[1])
        )
    ...
    # self.system => Represents Current Color's Requirements (calculated on the fly by the property we looked at).
    # system => The Target Terminal's Actual Ability (passed in as an argument from the outside environment).
